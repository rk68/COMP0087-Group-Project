...
for i in range(min(10, len(dataset))):  # Ensure not to exceed dataset length
    ...
    enhanced_prompt = f"""Below is a questions and the multiple choice options. Write a response that appropriately completes the request.\n\n### Question: {prompt}\n Options: {options_text}\n\n### Response: Let's think step by step."""
    ...
    if len(split_result) == 2:  # Ensure the split produced exactly two parts
...
print(f"Exact match score: {em_score*100}%")
